...
export function calculate(pricing, { packageId, addonIds = [], rush = false, tripMiles = null }, config = null) {
  // packageId may be 'none' when the customer is booking video/floor-plan/drone
  // only with no photo package attached.
  const pkg = packageId === 'none'
    ? null
    : pricing.photo_packages.find(p => p.id === packageId);
  if (packageId !== 'none' && !pkg) throw new Error('Unknown package: ' + packageId);
  ...
  // Trip charge — added when property is beyond the free-radius threshold.
  // Calculated as a % of the pretax subtotal so it scales with the job size.
  if (config && tripMiles !== null && tripMiles !== undefined) {
    const tc = config.service_area?.trip_charge;
    if (tc?.enabled && tripMiles > tc.free_miles) {
      const rate = tc.rate ?? 0.15;
      const fee = round2(subtotal * rate);
      const label = tc.label || 'Travel fee';
      lineItems.push({
        type: 'fee',
        id: 'trip',
        name: `${label} (${Math.round(tripMiles)} mi)`,
        amount: fee,
      });
      subtotal += fee;
    }
