...
// ---- Guest contact draft cookie ----
// Stores name/email/phone/brokerage between step 1 and the final POST.
// Signed with the same SESSION_SECRET to prevent tampering.
const CONTACT_COOKIE = 'df_contact';
const CONTACT_TTL_MS = 4 * 60 * 60 * 1000; // 4 hours

export function setContactDraft(res, { name, email, phone, brokerage, brokerageAddress }) {
  const payload = Buffer.from(JSON.stringify({ name, email, phone, brokerage, brokerageAddress })).toString('base64url');
  const sig = crypto.createHmac('sha256', secret()).update(payload).digest('base64url');
  res.cookie(CONTACT_COOKIE, `${payload}.${sig}`, {
    httpOnly: true, sameSite: 'lax',
    secure: (process.env.PUBLIC_URL || '').startsWith('https://'),
    maxAge: CONTACT_TTL_MS,
  });
}

export function readContactDraft(req) {
  const raw = req.cookies?.[CONTACT_COOKIE];
  if (!raw) return null;
...
export function clearContactDraft(res) { res.clearCookie(CONTACT_COOKIE); }
