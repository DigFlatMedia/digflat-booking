...
export async function exchangeCode(code) {
  ...
  // Also cache in process.env so it survives until next deploy without a file read.
  process.env.GOOGLE_REFRESH_TOKEN = data.refresh_token;
  console.log('[google] refresh_token obtained — save this as GOOGLE_REFRESH_TOKEN env var on Render to survive deploys.');
...
export async function getRefreshToken() {
  ...
  return stored?.refresh_token || process.env.GOOGLE_REFRESH_TOKEN || null;
}

// On startup: if GOOGLE_REFRESH_TOKEN env var is set but the token file doesn't exist,
// seed the file so the rest of the code works identically across deploys.
export async function initFromEnv() {
  const envToken = process.env.GOOGLE_REFRESH_TOKEN;
  if (!envToken) return;
  const stored = await readJSON(TOKEN_FILE, null);
  if (!stored?.refresh_token) {
    await writeJSON(TOKEN_FILE, { refresh_token: envToken, source: 'env', obtained_at: Date.now() });
    console.log('[google] Seeded token file from GOOGLE_REFRESH_TOKEN env var.');
  }
}
...
async function getAccessToken() {
  ...
  if (!refreshToken) throw new Error('Google Calendar not connected. Visit /admin/google to authorize.');
  const res = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
